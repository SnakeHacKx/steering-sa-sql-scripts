...
CREATE PROC PROC_ELIMINAR_REPORTE(
...
--PROCEDIMIENTO PARA MODIFICAR EL ESTADO DE LOS REPORTES 
...
BEGIN
	...
	BEGIN
		IF EXISTS(SELECT *FROM Mantenimiento WHERE Cod_reporte=@Cod_repote AND Fecha<=Format(GETDATE(),'yyyy-MM-dd'))--SI EL REPORTE TIENE MANTENIMIENTO REGISTRADO Y EL MANTENIMIENTO FUE HECHO EL REPORTE FUE ATENDIDO
			SET @Estado='ATENDIDO'
